...
def welcome_screen(self, root):
    ...
    storyBtn = Button(center_frame, text='Quiz Questions', width=12, height=2,
                      command=self.quizQuestionsStart)
    storyBtn.grid(row=1, column=2)

    moodBtn = Button(center_frame, text='Mood Booster', width=12, height=2,
                     command=lambda : self.screenSwitcher("moodbooster"))
